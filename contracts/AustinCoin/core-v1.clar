...
(define-data-var cityWallet principal 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-core-v1)
...
(define-public (register-user (memo (optional (string-utf8 50))))
  (let
    (
      ...
      (initialized (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-auth is-initialized))
    )
    ...
    (if (is-eq newId threshold)
      (let
        ...
        (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-auth activate-core-contract (as-contract tx-sender) activationBlockVal))
        (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-token activate-token (as-contract tx-sender) activationBlockVal))
...
(define-private (stack-tokens-at-cycle (user principal) (userId uint) (amountTokens uint) (startHeight uint) (lockPeriod uint))
  (let
    ...
    (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-token transfer amountTokens tx-sender (as-contract tx-sender) none))
    (print {
...
(define-private (claim-stacking-reward-at-cycle (user principal) (stacksHeight uint) (targetCycle uint))
  (let
    ...
    (if (> toReturn u0)
      (try! (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-token transfer toReturn tx-sender user none)))
      true
    )
...
(define-private (set-coinbase-thresholds)
  (let
    (
      (coinbaseAmounts (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-token get-coinbase-thresholds)))
    )
...
(define-private (mint-coinbase (recipient principal) (stacksHeight uint))
  (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-token mint (get-coinbase-amount stacksHeight) recipient))
)
...
(define-private (is-authorized-auth)
  (is-eq contract-caller 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-auth)
)
