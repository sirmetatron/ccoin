...
(define-fungible-token austincoin)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (ft-transfer? austincoin amount from to)
  )
)
...
(define-read-only (get-balance (user principal))
  (ok (ft-get-balance austincoin user))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply austincoin))
)
...
(define-public (activate-token (coreContract principal) (stacksHeight uint))
  (let
    (
      (coreContractMap (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-auth get-core-contract-info coreContract)))
    )
...
(define-data-var tokenUri (optional (string-utf8 256)) (some u"https://cdn.citycoins.co/metadata/austincoin.json"))
...
(define-public (mint (amount uint) (recipient principal))
  (let
    (
      (coreContract (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.austincoin-auth get-core-contract-info contract-caller)))
    )
    (ft-mint? austincoin amount recipient)
  )
)
...
(define-public (burn (amount uint) (owner principal))
  (begin
    ...
    (ft-burn? austincoin amount owner)
  )
)
